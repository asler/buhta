...
class BUHTA {
  postDBLoad(container) {
    ...
    const calibersArr = Object.keys(db.templates.items).filter(i => items[i]._parent === ammoParent).map(a => ({
      penetrationPower: items[a]._props.PenetrationPower,
      id: a,
      caliber: items[a]._props.Caliber,
      price: db.templates.handbook.Items.find(i => i.Id === a)?.Price,
      name: getName(a + " Name")
    }));
    ...
    const ignoreBotTypes = ["assault", "ravangezryachiyevent", "spiritspring", "spiritwinter", "infectedcivil",
      "infectedassault", "infectedlaborant", "infectedpmc", "infectedtagilla", "", "test", "gifter"];
    ...
    ignoreBotTypes.push(...(!config.sectant ? ["sectactpriestevent", "sectantoni", "sectantpredvestnik", "sectantpriest",
      "sectantwarrior", "sectantprizrak"] : []));
    
    const botTypes = Object.keys(db.bots.types).filter(k => !ignoreBotTypes.includes(k))
      .filter(t => {
        console.log(t,db.bots.types[t].health?.BodyParts)
        return db.bots.types[t].health?.BodyParts?.filter(h => h.Chest.max > 101)?.length;
      });
    ...
    const getMaxBodyHP = (b) => {
      const hp = b.health?.BodyParts?.filter(h => h.Chest.max > 101);
